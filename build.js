...
async function build() {
    for (const runModeName of Object.keys(runModes)) {
        const runMode = runModes[runModeName];
        fs.writeFileSync('externals.ts', readFile(`externals/${runMode.externals}.ts`)
            .replace(/--------\n\n/, "--------\n// DO NOT EDIT THIS FILE\n\n"));

        for (const isProduction of [false, true]) {
            let params = {
                entryPoints: ['./jstracetoix.ts'],
                outfile: runModeName + '/jstracetoix' + (isProduction ? '' : '-dev') + runMode.ext,
                minify: isProduction,
                sourcemap: isProduction,
                platform: runMode.platform,
                bundle: true,
                banner: { js: BANNER + `\n// Version: ${version}` },
                external: ['worker_threads'],
                format: runMode.format
            };
            if (runModeName === 'browser') {
